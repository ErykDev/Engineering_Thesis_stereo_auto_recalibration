...
sbm = cv2.StereoBM_create(blockSize=6)
...
filter = cv2.ximgproc.createDisparityWLSFilter(sbm)
...
while True:
    ...
    if not ret1:
        ...
        disparity = sbm.compute(img_rect1, img_rect2)
        filtered_disp = filter.filter(disparity, img_rect1)

        depth = disp_to_depth(disparity, 528.8050033826225, 12.03 * 0.1)
