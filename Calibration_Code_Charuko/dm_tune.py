...
imgLeft = cv2.imread(root + '/cam2_rectyfied/22.png', 0) #pair_img [0:photo_height,0:image_width] #Y+H and X+W
imgRight = cv2.imread(root + '/cam4_rectyfied/22.png', 0) #pair_img [0:photo_height,image_width:photo_width] #Y+H and X+W
...
loadah = plt.axes([0.7, 0.38, 0.15, 0.04]) #stepX stepY width height
buttonsp = Button(loadah, 'save plot', color=axcolor, hovercolor='0.975')

def save_disp(event):
    dmObject.write_png('file.png')

buttonsp.on_clicked(save_disp)
